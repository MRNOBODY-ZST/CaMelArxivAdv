...
- 基础设施基线：`backend/compose.yaml` 只有 MongoDB、MySQL 和 Redis，含不安全的开发明文值；没有 PostgreSQL、Kafka、MinIO 或 Mailpit。
...
- 消息与状态：Kafka + Redis；消息版本化、手动 ACK、幂等键和死信。
- Worker：Python 3.12、Pydantic、aiokafka/httpx、pytest、Ruff、MyPy。
...
4. **Source 解析**：安全下载/解包、TeX 发现、作者/邮箱/机构提取、证据与置信度、Kafka 幂等回写。
