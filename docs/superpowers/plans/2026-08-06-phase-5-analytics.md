...
**Acceptance:** PostgreSQL reference queries and the authenticated APIs reconciled on the real Phase 3/4 cohort. After independent review, canonical-author de-duplication, route/race reloads, user-directory authorization, allowlisted datasets, maintenance-window index safety, sequential query execution, explicit freshness states, missing distributions/rates, reduced motion, and security/export regressions were added. Backend (168), Worker (68), and frontend (34) tests plus lint, strict typing, builds, Compose/image contracts, desktop/mobile browser QA, export audit, privacy inspection, and container health checks passed. Temporary QA identities and their audit rows were removed after verification.
