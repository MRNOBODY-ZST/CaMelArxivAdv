...
- “检测图片加载（可选）”默认不勾选。它只在当前测试邮件中请求 `trackOpens=true`；关闭对话框或重新打开测试对话框会恢复为未勾选。
- 控件会显示当前回传地址及 `LOCAL_ONLY` 或 `PUBLIC_HTTPS_UNVERIFIED` 警告。配置无法读取或全局关闭时，只禁用该控件，不会阻止未跟踪的测试发送。
...
- 记录列表显示脱敏收件人、主题、账户、SMTP 结果、检测期和“尚无回传 / 检测到图片加载”等状态。详情显示记录 ID、到期时间、计数及最近的预取、图片代理、自动化或未分类回传；不会显示或加载 token、像素 URL。
