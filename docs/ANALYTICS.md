...
- 联系人统计先按 `paper_id,contact_id` 取最新映射，再应用域名/置信度筛选；不能先筛历史置信度再去重。
...
响应包含 `window.dateBasis=papers.imported_at`、`window.timezone=UTC` 和 `freshness`。每个比率指标同时返回 `numerator`、`denominator`、`value` 和定义，便于用 SQL 独立对账。

CSV 只导出聚合指标/序列，字符串执行 RFC 4180 双引号转义，并对 `= + - @` 开头的值加前置单引号，降低电子表格公式注入风险。导出写入 `ANALYTICS_EXPORT_CREATED` 审计，包含 Actor、视图、UTC 窗口和 Trace ID；不会包含完整邮箱。每张 ECharts 图可在浏览器导出白底 2× PNG。

## 查询与性能

Flyway V8 为导入日期、导入 Job、分类关系、最新提取、耗时分位、最新联系人映射、规则、操作用户和错误码建立专用索引。查询使用有界日期队列、`DISTINCT ON` 最新记录和 PostgreSQL 聚合/`percentile_cont`，不会在请求路径扫描原始追踪事件。
