...
public final class CampaignWorkflowRepository {
	...
	public Mono<PreflightRecord> preflight(UUID campaignId, Instant cooldownCutoff) {
		return databaseClient.sql("""
				...
				SELECT c.id, c.purpose, c.from_name, c.from_email, c.reply_to,
				       ...
				       mailbox.id AS mailbox_id, mailbox.enabled AS mailbox_enabled,
				...
				.map((row, metadata) -> new PreflightRecord(
						...
						row.get("reply_to", String.class), bool(row, "tracking_opens_enabled"),
	...
	public Mono<StateRecord> submitReview(UUID id, long expected, UUID actorId, byte[] digest) {
		return databaseClient.sql("""
				UPDATE campaigns SET status = 'READY_FOR_REVIEW', submitted_for_review_at = now(),
				    review_preflight_digest = :digest, review_preflight_at = now(),
				    status_changed_at = now(), status_changed_by = :actorId,
				    updated_by = :actorId, updated_at = now(), lock_version = lock_version + 1
				WHERE id = :id AND status IN ('DRAFT') AND lock_version = :expected
				RETURNING id, status, lock_version
				""").bind("digest", digest).bind("actorId", actorId).bind("id", id).bind("expected", expected)
				.map(this::mapState).one();
	}

	public Mono<StateRecord> approve(UUID id, long expected, UUID actorId, byte[] digest) {
		return databaseClient.sql("""
				UPDATE campaigns SET status = 'APPROVED', approved_at = now(), approved_by = :actorId,
				    rejected_at = NULL, rejected_by = NULL, rejection_reason = NULL,
				    review_preflight_digest = :digest, review_preflight_at = now(),
				    status_changed_at = now(), status_changed_by = :actorId,
				    updated_by = :actorId, updated_at = now(), lock_version = lock_version + 1
	...
	public Mono<StateRecord> schedule(
			UUID id, long expected, UUID actorId, Instant scheduledAt, byte[] digest
	) {
		return databaseClient.sql("""
				UPDATE campaigns SET status = 'SCHEDULED', scheduled_at = :scheduledAt,
				    review_preflight_digest = :digest, review_preflight_at = now(),
				    status_changed_at = now(), status_changed_by = :actorId,
				    updated_by = :actorId, updated_at = now(), lock_version = lock_version + 1
				WHERE id = :id AND status IN ('APPROVED') AND lock_version = :expected
				RETURNING id, status, lock_version
				""").bind("scheduledAt", scheduledAt).bind("digest", digest).bind("actorId", actorId)
				.bind("id", id).bind("expected", expected).map(this::mapState).one();
	}

	public Mono<StateRecord> start(UUID id, long expected, UUID actorId, byte[] digest) {
		return databaseClient.sql("""
				UPDATE campaigns SET status = 'RUNNING', started_at = coalesce(started_at, now()),
				    review_preflight_digest = :digest, review_preflight_at = now(),
				    status_changed_at = now(), status_changed_by = :actorId,
				    updated_by = :actorId, updated_at = now(), lock_version = lock_version + 1
				WHERE id = :id AND status IN ('APPROVED', 'SCHEDULED') AND lock_version = :expected
				RETURNING id, status, lock_version
				""").bind("digest", digest).bind("actorId", actorId).bind("id", id).bind("expected", expected)
				.map(this::mapState).one();
	}
	...
	public record PreflightRecord(
			...
			int perDayLimit, int perDomainHourLimit, UUID mailboxId, boolean mailboxEnabled,
